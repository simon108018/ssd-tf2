...
def convert_annotation_voc(year, image_id, list_file):
...
def convert_annotation_coco(image_id, list_file):
    in_file = open('VOCdevkit/coco/Annotations/%s.xml' % (image_id), encoding='utf-8')
    tree = ET.parse(in_file)
    root = tree.getroot()
...
if (dataname == 'voc2007') or (dataname == 'voc2012'):
    for year, image_set in sets:
        image_ids = open('VOCdevkit/VOC%s/ImageSets/Main/%s.txt' % (year, image_set)).read().strip().split()
        list_file = open('%s_%s.txt' % (year, image_set), 'w')
        for image_id in image_ids:
            list_file.write('%s/VOCdevkit/VOC%s/JPEGImages/%s.jpg' % (wd, year, image_id))
            convert_annotation_voc(year, image_id, list_file)
            list_file.write('\n')
        list_file.close()
else:
    for image_set in sets:
        image_ids = open('VOCdevkit/coco/ImageSets/Main/%s.txt' % (image_set)).read().strip().split()
        list_file = open('coco_%s.txt' % (image_set), 'w')
        for image_id in image_ids:
            list_file.write('%s/VOCdevkit/coco/JPEGImages/%s.jpg' % (wd, image_id))
            convert_annotation_coco(image_id, list_file)
            list_file.write('\n')
        list_file.close()